...
export default class UserController {
    static async userSignUp(req, res) {
        ...
        try {
            const emailVerificationToken = jwt.sign({
                ...
                process.env.JWT_ACCOUNT_VEIRIFICATION, { expiresIn: "72h" }
            );
            const data = {
    ...
    static async forgetPassword(req, res) {
        ...
        const foundUser = await _user.findOne({ where: { email } });
        if (foundUser) {
            const token = jwt.sign({ email: foundUser.email, _id: foundUser._id },
                foundUser.password, {
                    expiresIn: "24h",
                }
            );
    ...
    static async changeRoles(req, res, next) {
        ...
        try {
            ...
            if (role === roles.REQUESTER) {
                await _user.update({ role, manager: managers.DEFAULT_MANAGER }, {
                    where: { id: userId }
                })
            } else {
                await _user.update({ role, manager: "" }, {
                    where: { id: userId }
                })
            }
    ...
    static async emailVerification(req, res) {
        try {
            ...
            await _user.update({ isConfirmed: true }, { where: { email: req.decoded.email } });
