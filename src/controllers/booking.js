...
export default class RoomBooking {
  static async createBooking(req, res) {
    try {
      ...
      const booking = await roomBooking.create({ user_id,
  ...
  static async getAllBookings(req, res) {
    ...
    const myBooking = await roomBooking.findAll({ where: { user_id: userId,
      room_id: roomId } });
  ...
  static async updateBookings(req, res) {
    try {
      ...
      const booking = await roomBooking.findOne({where:
      { id: book_id,
        user_id: usersId}
      });
      if (!booking) {
  ...
  static async deleteBooking(req, res) {
    ...
    const { id } = req.user;
    const booking = await roomBooking.findOne({ where: { id: b_id, user_id: id } });
    if (!booking) {
