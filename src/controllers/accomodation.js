...
class Accommodation {
  ...
  static async like(req, res) {
    ...
    const savelike = await models.Like.create({ userId: id, accommodationId });
    return res.status(201).send({
  ...
  static async feedback(req, res) {
    ...
    const accommodation = await models.Accommodation.findOne({ where: { id: accommodationId } });
    if (!accommodation)
      return res.status(404).send({ message: ` Accommodation of ID "${accommodationId}" does not Exist !` });
    const relatedRooms = await models.Room.findAll({ where: { accommodation_id: accommodationId } });
    if (!relatedRooms)
      return res.status(404).send({ message: ` The Entered Accommodation doesn't have rooms!` });
    const relatedTrips = await models.Trips.findAll({ where: { requester_id: id } });
    if (relatedTrips) {
      for (let i = 0; i < relatedTrips.length; i++) {
        if (relatedTrips[i].status !== "Approved")
          return res.status(404).send({ message: `Your trip has to be approved first` });
        if (relatedTrips[i].accommodation_id == accommodationId) {
          const savefeedback = await models.Feedback.create({ userId: id, accommodationId, feedback });
          return res.status(201).send({
            message: `You commented on accommodation of ID  ${accommodationId}`,
            data: savefeedback,
          });
        }
        return res.status(404).send({ message: `Accommodation entered is different from the one on your trip` });
      }
    }
    res.status(403).send({ message: `To Comment On an Accommodation, you have to trip with us first` });
  }
}
