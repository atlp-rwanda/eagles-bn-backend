...
export default class Trip {
  ...
  static async LatestRemember(req, res) {
    ...
    if (!user.remember_travel) {
      return res.status(400)
        .json({
          status: 400,
          message: 'Remember travel is OFF'
        });
    }
    ...
    if (!latestTripValues) {
      return res.status(404)
        .json({
          status: 404,
          message: 'No last trip available'
        });
    }
  ...
  static async latestTrips(req, res) {
    ...
    return onSuccess(res, 200, 'Latest trips', trips);
  }
}
