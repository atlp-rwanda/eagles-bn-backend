...
describe(` POST /api/rooms/1/booking/`, () => {
  ...
  it("it should creates a booking", async () => {
    ...
    const accessToken = await signAccessToken({ id: userData.id, email: 'nklbigone@gmail.com' });
  ...
  it("it should not creates a booking", async () => {
    const accessToken = await signAccessToken({ id: "welcome to my bed", email: 'faking' });
    const res = await chai
      .request(app)
      .post(`/api/rooms/1/booking/`)
      .set('auth-token', accessToken)
  ...
  it("it should get all a booking", async () => {
    const userData = await User.create(userInfo);
    const accessToken = await signAccessToken({ id: userData.id, email: 'faking' });
    const res = await chai
      .request(app)
  ...
  it("it should update a booking", async () => {
    const userData = await User.create(userInfo);
    const bookingKing = await Booking.create({ ...creatBooking, user_id: userData.id, room_id: 1 });
    const accessToken = await signAccessToken({ id: userData.id, email: 'faking' });
    const res = await chai
      .request(app)
  ...
  it("it should delete a booking ", async () => {
    const userData = await User.create(userInfo);
    const bookingKing = await Booking.create({ ...creatBooking,
      user_id: userData.id, room_id: 1 });
    const accessToken = await signAccessToken({ id: userData.id, email: 'faking' });
    const res = await chai
      .request(app)
